...
- Power BI Desktop is open with the `Customer Lookup` and `Product Lookup` tables already loaded from Handout 02.
...
## Part 0: Load the Calendar Lookup Table

Follow these explicit steps to import the calendar CSV into Power Query Editor before doing anything else in this handout.

### Steps

1. If Power Query Editor is not open, on the Power BI **Home** ribbon click **Transform Data** to open it.
2. On the **Home** ribbon of Power Query Editor, click **New Source** > **Text/CSV**.
3. Browse to the `AdventureWorks Raw Data` folder and select `AdventureWorks Calendar Lookup.csv`.
4. Click **Open**, then in the preview window click **OK** to load it into Power Query Editor.
5. In the **Queries** pane on the left, double-click the new query name and rename it to `Calendar Lookup`.
6. Press **Enter**.

### Verify the Date Column

1. Click the `Date` column header.
2. Confirm the data type icon shows the **calendar (Date)** icon. If it does not, click the icon and select **Date**.
3. The table should contain a single `Date` column.

> **Do not click Close & Apply yet.** Stay in Power Query Editor for the rest of this handout.
...
```text
   ...
   UK locale (dd/mm/yyyy):  day=13, month=01, year=2023  ✅
   US locale (mm/dd/yyyy):  month=13 → invalid          ❌  Error
```
