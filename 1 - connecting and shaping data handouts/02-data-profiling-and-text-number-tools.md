...
- Power BI Desktop is installed and open.
...
## Part 0: Load the Customer Lookup Table

Before doing anything else in this handout, follow these explicit steps to load the Customer Lookup CSV into Power Query Editor and prepare it.

### Steps — Load the CSV

1. Open Power BI Desktop.
2. On the **Home** ribbon, click **Get Data** > **Text/CSV**.
3. Browse to the `AdventureWorks Raw Data` folder and select `AdventureWorks Customer Lookup.csv`.
4. Click **Open**.
5. In the preview window, click **Transform Data**. (Do **not** click Load — we want to inspect the data in Power Query Editor first.)

### Steps — Rename the Query

1. In the **Queries** pane on the left of Power Query Editor, double-click the query name (`AdventureWorks Customer Lookup`).
2. Rename it to `Customer Lookup`.
3. Press **Enter**.

### Steps — Verify Column Headers and Data Types

Scroll across the table from left to right. Confirm each column has the correct data type by looking at the icon to the left of the column name:

| Column | Expected Data Type |
|---|---|
| `CustomerKey` | Whole Number (`123`) |
| `Prefix` | Text (`ABC`) |
| `FirstName` | Text |
| `LastName` | Text |
| `BirthDate` | Date (calendar icon) |
| `MaritalStatus` | Text |
| `Gender` | Text |
| `EmailAddress` | Text |
| `AnnualIncome` | Whole Number (you may optionally change this to **Currency** / Fixed Decimal Number) |
| `TotalChildren` | Whole Number |
| `EducationLevel` | Text |
| `Occupation` | Text |
| `HomeOwner` | Text |

If any data type is wrong, click the type icon in the column header and select the correct type.

> **Do not click Close & Apply yet.** Keep Power Query Editor open — the rest of this handout works inside it.
...
> **Heads up — wrong-tab demonstration (intentional mistake):** It is very easy to use the wrong ribbon tab here. Try the following first to see what happens, then fix it.
>
> 1. Click the `Prefix` column header to select it.
> 2. On the **Add Column** ribbon, click **Format** > **Capitalize Each Word**.
> 3. Notice that a **new column** named `Capitalize Each Word` is added at the far right of the table — the original `Prefix` column is unchanged.
> 4. In the **Applied Steps** pane on the right, click the **X** next to the latest step (the new column add) to delete it.
>
> Now do it correctly:
...
4. Click the `FirstName` column header. Hold **Shift** and click the `LastName` column header to select both columns at the same time.
5. On the **Transform** ribbon, click **Format** > **Capitalize Each Word**.
6. Power Query merges the transformation into a single applied step. Click the latest applied step and look at the M code in the formula bar — you should see `Text.Proper` applied to all three columns (`Prefix`, `FirstName`, `LastName`) in one expression.

**Expected result:** `Prefix`, `FirstName`, and `LastName` are now in proper case (for example, `Mr. Jon Yang`).
...
### Load the Product Lookup Table

The Number-Specific Tools demos and Exercise 4 use the `Product Lookup` table. Load it now if you have not already.

1. On the **Home** ribbon of Power Query Editor, click **New Source** > **Text/CSV**.
2. Browse to the `AdventureWorks Raw Data` folder and select `AdventureWorks Product Lookup.csv`.
3. Click **Open**, then click **OK** in the preview window to load it into Power Query Editor.
4. In the **Queries** pane, double-click the new query name and rename it to `Product Lookup`.
5. Scroll across the columns and confirm the data types — in particular:
   - `ProductKey` → Whole Number
   - `ProductName` → Text
   - `ProductPrice` → Fixed Decimal Number (currency). If it shows as **Decimal Number**, click the type icon and change it to **Fixed Decimal Number**.
...
**Example — Maximum product price:**

1. Select the `ProductPrice` column.
2. On the **Transform** ribbon, click **Statistics** > **Maximum**.
3. The maximum is approximately **$3,578.27** — confirming AdventureWorks sells some high-end items.
4. Delete the applied step.

---

### Demo: Rounding Numbers (Transform vs. Currency)

This is a quick look at the **Rounding** tools. You will not keep this transformation — it is for awareness only.

1. With the `ProductPrice` column selected, on the **Transform** ribbon click **Rounding** > **Round...**.
2. In the dialog, enter `2` for the number of decimal places.
3. Click **OK**. Notice the column values are now rounded to two decimal places.
4. Delete the **Rounded Off** applied step to revert. (If your data type was changed during this demo, also delete any **Changed Type** step that was added so the column returns to its original Currency / Fixed Decimal Number type.)

> **Why we revert this:** The `ProductPrice` column is already a Currency / Fixed Decimal Number type, which is what we want for the model.
